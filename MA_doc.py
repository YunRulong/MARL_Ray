...
#         *,  # 必须使用关键字参数格式
#         policy: PolicyID,
#         train_batch,  # 或 Dict[str, TensorType]
#         result: dict,
#         algorithm,  # 通过类型提示确保正确引用
#         **kwargs
#     ) -> None:
#         # 确保policy对象正确获取
#         policy = algorithm.get_policy(policy)
        
#         # 获取梯度信息
#         grad_info = policy.model.gradient_info
        
#         # 输出到控制台
#         print("\n===== Layer Gradient Statistics =====")
#         for layer_name, stats in grad_info.items():
#             print(f"{layer_name:15} | Max: {stats['max']:.4e} | Mean: {stats['mean']:.4e} | Min: {stats['min']:.4e}")
    ...
    def _register_gradient_hooks(self):
        ...
        for name, module in self.named_children():
            if isinstance(module, (nn.Conv2d, nn.Linear, nn.BatchNorm2d, nn.LayerNorm)):
                module.register_backward_hook(make_hook(name))
